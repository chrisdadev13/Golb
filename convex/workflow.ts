...
export const generateBlocksWorkflow = workflow.define({
  ...
  handler: async (step, args) => {
    try {
      ...
      const course = await step.runQuery(
        internal.workflow.getCourseBySectionId,
        {
          sectionId: args.sectionId,
        },
      );
...
export const generateVideoCourseAction = internalAction({
  ...
  handler: async (_, args) => {
    ...
    const { r2_url, r2_filename } = data as {
      r2_url: string;
      r2_filename: string;
    };
...
async function processDocument({ documentUrl }: { documentUrl: string }) {
  ...
  handler: async (ctx, args) => {
    ...
    if (!settings || settings.notifyWhenFlashcardSetIsReady) {
      await ctx.scheduler.runAfter(
        0,
        internal.emails.sendFlashcardsReadyEmail,
        {
          userEmail: user.email,
          userName: user.name,
